// AUTO-GENERATED - DO NOT EDIT
// Generated by @nocturne/remote-function-codegen
// Source: openapi.json

import { getRequestEvent, query, command } from '$app/server';
import { error, redirect } from '@sveltejs/kit';
import { z } from 'zod';
import { PatientRecordSchema, PatientDeviceSchema, PatientInsulinSchema } from '$lib/api/generated/schemas';
import { type PatientRecord, type PatientDevice, type PatientInsulin } from '$api';

/** Get or create the patient record */
export const getPatientRecord = query(async () => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    return await apiClient.patientRecord.getPatientRecord();
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.getPatientRecord:', err);
    throw error(500, 'Failed to get patient record');
  }
});

/** Update the patient record */
export const updatePatientRecord = command(PatientRecordSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    const result = await apiClient.patientRecord.updatePatientRecord(request as PatientRecord);
    await Promise.all([
      getPatientRecord(undefined).refresh()
    ]);
    return result;
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.updatePatientRecord:', err);
    throw error(500, 'Failed to update patient record');
  }
});

/** Get all patient devices */
export const getDevices = query(async () => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    return await apiClient.patientRecord.getDevices();
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.getDevices:', err);
    throw error(500, 'Failed to get devices');
  }
});

/** Create a new patient device */
export const createDevice = command(PatientDeviceSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    const result = await apiClient.patientRecord.createDevice(request as PatientDevice);
    await Promise.all([
      getDevices(undefined).refresh()
    ]);
    return result;
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.createDevice:', err);
    throw error(500, 'Failed to create device');
  }
});

/** Update a patient device */
export const updateDevice = command(z.object({ id: z.string(), request: PatientDeviceSchema }), async ({ id, request }) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    const result = await apiClient.patientRecord.updateDevice(id, request as PatientDevice);
    await Promise.all([
      getDevices(undefined).refresh()
    ]);
    return result;
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.updateDevice:', err);
    throw error(500, 'Failed to update device');
  }
});

/** Delete a patient device */
export const deleteDevice = command(z.string(), async (id) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    await apiClient.patientRecord.deleteDevice(id);
    await Promise.all([
      getDevices(undefined).refresh()
    ]);
    return { success: true };
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.deleteDevice:', err);
    throw error(500, 'Failed to delete device');
  }
});

/** Get all patient insulins */
export const getInsulins = query(async () => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    return await apiClient.patientRecord.getInsulins();
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.getInsulins:', err);
    throw error(500, 'Failed to get insulins');
  }
});

/** Create a new patient insulin */
export const createInsulin = command(PatientInsulinSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    const result = await apiClient.patientRecord.createInsulin(request as PatientInsulin);
    await Promise.all([
      getInsulins(undefined).refresh()
    ]);
    return result;
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.createInsulin:', err);
    throw error(500, 'Failed to create insulin');
  }
});

/** Update a patient insulin */
export const updateInsulin = command(z.object({ id: z.string(), request: PatientInsulinSchema }), async ({ id, request }) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    const result = await apiClient.patientRecord.updateInsulin(id, request as PatientInsulin);
    await Promise.all([
      getInsulins(undefined).refresh()
    ]);
    return result;
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.updateInsulin:', err);
    throw error(500, 'Failed to update insulin');
  }
});

/** Delete a patient insulin */
export const deleteInsulin = command(z.string(), async (id) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
    await apiClient.patientRecord.deleteInsulin(id);
    await Promise.all([
      getInsulins(undefined).refresh()
    ]);
    return { success: true };
  } catch (err) {
    const status = (err as any)?.status;
    if (status === 401) { const { url } = getRequestEvent(); throw redirect(302, `/auth/login?returnUrl=${encodeURIComponent(url.pathname + url.search)}`); }
    if (status === 403) throw error(403, 'Forbidden');
    console.error('Error in patientRecord.deleteInsulin:', err);
    throw error(500, 'Failed to delete insulin');
  }
});
