...
export const create = command(BolusCalculationSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
...
export const update = command(z.object({ id: z.string(), request: BolusCalculationSchema }), async ({ id, request }) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
