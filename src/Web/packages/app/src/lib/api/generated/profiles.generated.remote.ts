...
export const createTherapySettings = command(TherapySettingsSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
...
export const updateTherapySettings = command(z.object({ id: z.string(), request: TherapySettingsSchema }), async ({ id, request }) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
...
export const createBasalSchedule = command(BasalScheduleSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
...
export const updateBasalSchedule = command(z.object({ id: z.string(), request: BasalScheduleSchema }), async ({ id, request }) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
