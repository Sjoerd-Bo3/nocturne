...
export const create = command(CalibrationSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
...
export const update = command(z.object({ id: z.string(), request: CalibrationSchema }), async ({ id, request }) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
