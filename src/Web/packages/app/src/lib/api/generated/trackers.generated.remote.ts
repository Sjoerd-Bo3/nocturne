...
export const createDefinition = form(CreateTrackerDefinitionRequestSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
...
export const updateDefinition = form(z.object({ id: z.string(), request: UpdateTrackerDefinitionRequestSchema }), async ({ id, request }) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
