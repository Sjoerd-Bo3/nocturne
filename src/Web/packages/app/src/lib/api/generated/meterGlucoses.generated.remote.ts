...
export const create = command(MeterGlucoseSchema, async (request) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
...
export const update = command(z.object({ id: z.string(), request: MeterGlucoseSchema }), async ({ id, request }) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
