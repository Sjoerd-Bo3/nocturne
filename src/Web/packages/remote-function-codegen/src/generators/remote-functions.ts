...
import { ${kitImports.join(', ')} } from '@sveltejs/kit';
...
function generateFunction(op: OperationInfo, tag: string, allOpsInTag: OperationInfo[]): string {
  ...
  if (op.remoteType === 'query') {
    if (op.isBatch) {
      return generateBatchQueryFunction(op, functionName, clientProperty, methodName);
    }
    return generateQueryFunction(op, functionName, clientProperty, methodName);
  } else if (op.remoteType === 'form') {
    return generateMutationFunction(op, functionName, clientProperty, methodName, tag, allOpsInTag, 'form');
  } else {
    return generateMutationFunction(op, functionName, clientProperty, methodName, tag, allOpsInTag, 'command');
  }
}
...
function generateMutationFunction(
  ...
  if (schemaArg) {
    return `${comment}export const ${functionName} = ${wrapperName}(${schemaArg}, async (${paramList}) => {
  const { locals } = getRequestEvent();
  const { apiClient } = locals;
  try {
${apiCall}
